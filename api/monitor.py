...
class ToolMonitor:
    ...
    def report_tool(self, tool_name: str, args: Dict[str, Any] = None):
        """报告工具开始执行"""
        self._emit("tool_start", f"子智能体正在执行工具: {tool_name}", {"tool_name": tool_name, "args": args})

    def report_main_agent_tool(self, tool_name: str, args: Dict[str, Any] = None):
        """报告主智能体正在调用的普通工具 (如 execute, generate_markdown)"""
        self._emit("main_tool_start", f"主智能体正在执行工具: {tool_name}", {"tool_name": tool_name, "args": args})

    def report_todos_update(self, todos: list):
        """专门报告待办事项 (to_do-list) 的更新，触发前端的 To_do 侧边栏/面板"""
        self._emit("todos_updated", "任务规划更新", {"todos": todos})
    ...
    def report_interrupt(self, action_requests: list):
        """下发人工审批请求"""
        self._emit("hitl_require_approval", "系统已挂起，等待人工审核", {"action_requests": action_requests})

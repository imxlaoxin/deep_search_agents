...
@tool
def get_assistant_list(config: RunnableConfig = None) -> str:
    """
    ...
    # 埋点监控：记录工具调用行为
    monitor.report_tool("get_assistant_list", args={"cur_subagent_id": cur_subagent_id})
...
def create_ask_delete(
        ...
        reflection_on_previous: Annotated[
            str, "强制要求：如果上一次提问没有得到满意答案，必须在此填写反思（如：上次问得太宽泛，这次改为具体问某某参数）"] = "初次提问无需反思",
    ...
    cur_subagent_id = f"{thread_id}_{stable_subagent_id}"
    current_count = _rag_counters.get(cur_subagent_id, 0)

    # 物理拦截：最多允许 LLM 多跳追问 2 次
    if current_count >= 2:
        _rag_counters[cur_subagent_id] = 0
        monitor.report_tool("create_ask_delete-system_warning", {'警报': '触发物理熔断，终止RAG追问'})
        return "【系统强制拦截】知识库追问已达3次上限！禁止再调用提问工具！请立即停止查询，综合现有已知信息输出结论。"

    _rag_counters[cur_subagent_id] = current_count + 1

    # 埋点监控：记录提问信息
    monitor.report_tool(tool_name="create_ask_delete", args={
            "助手名称": assistant_name,
            "查询问题": question,
            "反思内容": reflection_on_previous,
            "追问次数": f'{current_count}',

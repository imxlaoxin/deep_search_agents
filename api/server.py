...
from agent.main_agent import run_deep_agent, resume_deep_agent
...
async def approve_task(request: ApproveRequest):
    ...
    人工审批回调接口 (HITL)
    """
    # 异步触发唤醒流，不阻塞当前 HTTP 请求
    asyncio.create_task(resume_deep_agent(request.thread_id, request.decisions))
...
async def run_task(request: TaskRequest):
    ...
    # 注意：这里使用 asyncio.create_task 触发（而不是使用async.run方法），由 main_agent 内部负责实时推送

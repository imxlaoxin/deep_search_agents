...
def resolve_path(filename: str, session_dir: Optional[str] = None) -> str:
    ...
    Returns:
        str: 解析后的绝对路径
    """
    # 防范大模型导致的路径嵌套灾难（如 output/session_abc/output/session_abc）
    if session_dir:
        session_path = Path(session_dir).resolve()
        session_name = session_path.name
        # 如果传入的 filename 里包含了重复的 session 文件夹名字，直接进行字符串清洗剥离
        while f"{session_name}/{session_name}" in filename.replace("\\", "/"):
            filename = filename.replace(f"{session_name}/{session_name}", session_name)
        while f"output/{session_name}/output/{session_name}" in filename.replace("\\", "/"):
            filename = filename.replace(f"output/{session_name}/output/{session_name}", f"output/{session_name}")
